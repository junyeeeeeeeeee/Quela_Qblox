...
from numpy import std, arange, array, linspace, pi, average, mean, sign
...
def Ramsey_coupler(QD_agent:QDmanager,
    ...
    if run:
        ...
        meas_ctrl.settables([Para_free_Du,Cctrl[target_coupler]])
        ...
        analysis_result[q] = Basic2DAnalysis(tuid=ramsey_ds.attrs["tuid"], dataset=ramsey_ds).run()
...
def ramsey_coupler_executor(QD_agent:QDmanager,
    ...
    if run:
        ...
        Ramsey_results, T2_us, average_actual_detune = Ramsey_coupler(QD_agent,meas_ctrl,Cctrl,target_coupler=target_coupler,flux_center=flux_center,flux_span=flux_span,flux_points=flux_pts,arti_detune=artificial_detune,freeduration=freeDura,n_avg=avg_n,q=specific_qubits,ref_IQ=QD_agent.refIQ[specific_qubits],points=pts,run=True,exp_idx=ith,data_folder=specific_folder)
        ...
        Ramsey_results, _, average_actual_detune = Ramsey_coupler(QD_agent,meas_ctrl,Cctrl,target_coupler=target_coupler,flux_center=flux_center,flux_span=flux_span,flux_points=flux_pts,arti_detune=artificial_detune,freeduration=freeDura,n_avg=1000,q=specific_qubits,ref_IQ=QD_agent.refIQ[specific_qubits],points=100,run=False)
        this_t2_us = 0
...
if __name__ == "__main__":
    ...
    for qubit in ro_elements:
        ...
        for ith_histo in range(ro_elements[qubit]["histo_counts"]):
            ...
            ramsey_results, this_t2_us, average_actual_detune = ramsey_coupler_executor(QD_agent,cluster,meas_ctrl,target_coupler,flux_center=target_center,flux_span=target_bias_span,flux_pts=target_pts,Fctrl=Fctrl,specific_qubits=qubit,artificial_detune=ro_elements[qubit]["detune"],freeDura=ro_elements[qubit]["evoT"],ith=ith_histo,run=execution)
