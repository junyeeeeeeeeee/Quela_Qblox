...
from Modularize.support.Pulse_schedule_library import Cryoscope_ramsey_sche, set_LO_frequency, pulse_preview, IQ_data_dis, dataset_to_array, Cryoscope_data, Just_plot
...
def Ramsey(QD_agent:QDmanager,
    ...
    Para_z_Du = ManualParameter(name="z_Duration", unit="s", label="Time")
    Para_z_Du.batched = True
    gap = (set_T2_time)*1e9 // points + (((set_T2_time)*1e9 // points) %4) # multiple by 4 ns
    samples = arange(4e-9,set_T2_time,gap*1e-9)
    
    sche_func= Cryoscope_ramsey_sche
    sched_kwargs = dict(
        q=q,
        pi_amp={str(q):qubit.rxy.amp180()},
        target_z=target_z,
        target_z_amp=target_z_amp,
        set_T2_time=set_T2_time,
        z_gap=z_gap,
        New_fxy=New_fxy,
        z_time=Para_z_Du,
    ...
    if run:
        ...
        meas_ctrl.settables(Para_z_Du)
        meas_ctrl.setpoints(samples)
        
        
        Cyroscope_ds = meas_ctrl.run('Cyroscope')

        # Save the raw data into netCDF
        Data_manager().save_raw_data(QD_agent=QD_agent,ds=Cyroscope_ds,label=exp_idx,qb=q,exp_type='T2',specific_dataFolder=data_folder)
        
        I,Q= dataset_to_array(dataset=Cyroscope_ds,dims=1)
        data= IQ_data_dis(I,Q,ref_I=ref_IQ[0],ref_Q=ref_IQ[1])  

        data_fit= Cryoscope_data(data=data,Z_Du=samples)
        ...
        sched_kwargs['z_time']= sweep_para.reshape(sweep_para.shape or (1,))
    ...
    for qubit in ro_elements:
        ...
        for ith_histo in range(ro_elements[qubit]["histo_counts"]):
            ...
            ramsey_results, this_t2_us, average_actual_detune = ramsey_executor(QD_agent,
                                                                                ...
                                                                                target_z_amp=target_z_amp,
                                                                                z_gap=z_gap,
                                                                                set_T2_time=ro_elements[qubit]["T2_time"],
                                                                                Fctrl=Fctrl,
        ...
        if execution:
            ...
            if ro_elements[qubit]["histo_counts"] == 1:
                mean_T2_us = 0
                sd_T2_us = 0
                Just_plot(ramsey_results[qubit],P_rescale=False,Dis=None)
            else:
