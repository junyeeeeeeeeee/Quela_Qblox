...
def qualityFit_executor(QD_agent:QDmanager,meas_ctrl:MeasurementControl,ro_amp:float,specific_qubits:str,ro_span_Hz:float=10e6,run:bool=True,fpts=600,n_avg:int=10, data_folder:str=""):
...
def generate_data_folder(target_q:str, additional_name:str="")->str:
    ...
    if not os.path.exists(Quality_folder_path):
...
if __name__ == "__main__":
    ...
    for qubit in ro_elements:
        ...
        dBm_output = list(dBm_bySA - ro_elements[qubit]["ro_atte"] - RT_real_atte)
        ...
        for ro_atte in ro_elements[qubit]["ro_atte"]:
            ...
            positional_result = qualityFit_executor(QD_agent,meas_ctrl,ro_amp=ro_elements[qubit]["ro_amp"],specific_qubits=qubit,ro_span_Hz=3e6,run=True)[qubit]
            ...
            positional_result = qualityFit_executor(QD_agent,meas_ctrl,ro_amp=ro_elements[qubit]["ro_amp"],specific_qubits=qubit,ro_span_Hz=3*peak_width,run=True,data_folder=this_qubit_exp_folder)[qubit]
        #     CS_results = qualityFit_executor(QD_agent=QD_agent,meas_ctrl=meas_ctrl,specific_qubits=qubit,ro_amp=ro_elements[qubit]["ro_amp"],run = execution,ro_span_Hz=3*peak_width,data_folder=this_qubit_exp_folder)
        #     Fctrl[qubit](0)
        #     cluster.reset()
