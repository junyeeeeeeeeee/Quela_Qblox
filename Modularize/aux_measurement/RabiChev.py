...
from Modularize.support.Pulse_schedule_library import rabi_chev, set_LO_frequency, pulse_preview, IQ_data_dis, dataset_to_array, Rabi_fit_analysis, Fit_analysis_plot

def Rabi_chev(QD_agent:QDmanager,meas_ctrl:MeasurementControl,XY_amp:float=0.5, XY_duration:float=20e-9, IF:int=150e6,n_avg:int=300,points:int=100,run:bool=True,XY_theta:str='X_theta',Rabi_type:str='PowerRabi',q:str='q1',Experi_info:dict={},ref_IQ:list=[0,0], fpts:int=20):
    analysis_result = {}
    sche_func= rabi_chev
    ...
    if Rabi_type.lower() in ['timerabi', 'tr']:
        ...
        samples = arange(0,XY_duration,gap*1e-9)
    ...
    drive_freq = ManualParameter(name="xy_freq", unit="Hz", label="driving pulse frequency")
    drive_freq.batched = False
    f01_samples = linspace(qubit_info.clock_freqs.f01()-2*IF/3, qubit_info.clock_freqs.f01()+2*IF/3, fpts)
    sched_kwargs = dict(
        q=q,
        frequencies=drive_freq,
        XY_amp=Para_XY_amp,
    ...
    if run:
        ...
        meas_ctrl.settables([Sweep_para,drive_freq])
        meas_ctrl.setpoints_grid((samples,f01_samples))
    
       
        rabi_ds = meas_ctrl.run("Rabi-Chevron")
...
def rabi_chev_executor(QD_agent:QDmanager,cluster:Cluster,meas_ctrl:MeasurementControl,Fctrl:dict,specific_qubits:str,XYamp_max:float=0.5,XYdura_max:float=20e-9,which_rabi:str='power',run:bool=True,pts:int=100,avg_times:int=500,xy_IF:float=150e6,fpts:int=20):
    ...
    if run:
        ...
        Rabi_results = Rabi_chev(QD_agent,meas_ctrl,Rabi_type=exp_type,q=specific_qubits,ref_IQ=QD_agent.refIQ[specific_qubits],IF=xy_IF,run=True,XY_amp=XYamp_max,XY_duration=XYdura_max,points=pts,n_avg=avg_times,fpts=fpts)
        ...
        Rabi_results = Rabi_chev(QD_agent,meas_ctrl,Rabi_type=exp_type,q=specific_qubits,ref_IQ=QD_agent.refIQ[specific_qubits],IF=xy_IF,run=False,XY_amp=XYamp_max,XY_duration=XYdura_max,fpts=fpts)
        return 0, 0
...
if __name__ == "__main__":
    ...
    for qubit in ro_elements:
        ...
        rabi_results[qubit], trustable = rabi_chev_executor(QD_agent,cluster,meas_ctrl,Fctrl,qubit,run=execution,XYdura_max=pi_duration,XYamp_max=pi_amp_max,which_rabi=rabi_type,avg_times=avg_n,xy_IF=xy_if,fpts=freq_pts)
        cluster.reset()
