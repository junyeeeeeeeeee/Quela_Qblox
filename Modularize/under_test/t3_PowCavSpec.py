...
def PowerDep_spec(QD_agent:QDmanager,meas_ctrl:MeasurementControl,ro_elements:dict,ro_p_min:float=0.01,ro_p_max:float=0.5,p_points:int=20,n_avg:int=100,run:bool=True,Experi_info:dict={})->Dataset:
    ...
    if run:
        ...
        meas_ctrl.setpoints_grid((freq_datapoint_idx,ro_p_samples))
        ...
        try: 
            plot_powerCavity_S21(nc_file, ro_elements)
        except:
    ...
    for q in original_rof:
        qubit_info = QD_agent.quantum_device.get_element(q)
        qubit_info.clock_freqs.readout(original_rof[q]+1e6)
    return rp_ds

def plot_powerCavity_S21(PC_nc_file:str, ro_elements:dict):
    """
...
def powerCavity_executor(QD_agent:QDmanager,meas_ctrl:MeasurementControl,Fctrl:dict,qubits:dict,ro_span_Hz:float=3e6,max_power:float=0.7,run:bool=True,sweet_spot:bool=False,fpts:int=30,ppts:int=30,avg_n:int=100):
    ...
    if run:
        ...
        PD_ds = PowerDep_spec(QD_agent,meas_ctrl,ro_elements, ro_p_max=max_power, p_points=ppts, n_avg=avg_n)
        if sweet_spot:
