...
def Ramsey(QD_agent:QDmanager,meas_ctrl:MeasurementControl,freeduration:float,arti_detune:int=0,IF:int=130e6,n_avg:int=1000,points:int=101,run:bool=True,q='q1', ref_IQ:list=[0,0],Experi_info:dict={},exp_idx:int=0):
    ...
    if run:
        ...
        Data_manager().save_raw_data(QD_agent=QD_agent,ds=ramsey_ds,histo_label=exp_idx,qb=q,exp_type='T2')
...
def ramsey_executor(QD_agent:QDmanager,meas_ctrl:MeasurementControl,Fctrl:dict,specific_qubits:str,artificial_detune:float=0e6,freeDura:float=30e-6,histo_counts:int=1,run:bool=True):
    ...
    if run:
        ...
        for ith in range(histo_counts):
            ...
            Ramsey_results, T2_us, average_actual_detune= Ramsey(QD_agent,meas_ctrl,arti_detune=artificial_detune,freeduration=freeDura,n_avg=1000,q=specific_qubits,ref_IQ=QD_agent.refIQ[specific_qubits],points=100,run=True,exp_idx=ith)
            Fctrl[specific_qubits](0.0)
            
            T2_us_rec.append(T2_us[specific_qubits])
            detune_rec.append(average_actual_detune[specific_qubits])
        mean_T2_us = round(mean(array(T2_us_rec)),1)
        sd_T2_us = round(std(array(T2_us_rec)),1)
        if histo_counts == 1:
            Fit_analysis_plot(Ramsey_results[specific_qubits],P_rescale=False,Dis=None)
        # set the histo save path
        else:
            Data_manager().save_histo_pic(QD_agent,{str(specific_qubits):T2_us},specific_qubits,mode="t2",T1orT2=f"{mean_T2_us}+/-{sd_T2_us}")
    else:
...
if __name__ == "__main__":
    ...
    ro_elements = {
        "q0":{"detune":0e6,"evoT":5e-6,"histo_counts":1}
    }
