...
def xyf_calibrator(QD_agent:QDmanager,cluster:Cluster,meas_ctrl:MeasurementControl,Fctrl:dict,specific_qubit:str,max_evoT:float,execution:bool=True,plot_cali_result:bool=False):
    Trustable = False
    avg_N = 500
    _, _, detune = ramsey_executor(QD_agent,cluster,meas_ctrl,Fctrl,specific_qubit,artificial_detune=0e6,freeDura=max_evoT,histo_counts=1,run=execution,plot=False,avg_n=avg_N)
    if execution:
        ...
        for arti_detune in to_cali_detune:
            ...
            ramsey_results, _, average_actual_detune = ramsey_executor(QD_agent,cluster,meas_ctrl,Fctrl,specific_qubit,artificial_detune=arti_detune,freeDura=max_evoT,histo_counts=1,run=execution,plot=False,avg_n=avg_N)
