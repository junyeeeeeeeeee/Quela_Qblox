...
from qblox_drive_AS.support import compose_para_for_multiplexing, QDmanager
...
def active_reset(QD_agent:QDmanager, schedule:Schedule, q:str, acq_idx:int, acq_channel:int, R_amp:dict, R_duration:dict, R_integration:dict, R_inte_delay:dict, pi_amp:dict, pi_Du:float, ro_ref_pulse:Schedule=None, acq_threshold:float=None, acq_rotation:float=None):
    
    if acq_channel == 0:
        spec_pulse = Readout(schedule,q,R_amp,R_duration)
    else:
        spec_pulse = ro_ref_pulse
        Multi_Readout(schedule,q,spec_pulse,R_amp,R_duration)

    Integration(schedule,q,R_inte_delay[q],R_integration,spec_pulse,acq_idx,acq_channel,trace_recordlength=0,RO_protocol="ThresholdedAcquisition", acq_threshold=acq_threshold,acq_rotation=acq_rotation)

    sub_schedule = Schedule("conditional x")
    QD_agent.Waveformer.X_pi_p(sub_schedule, pi_amp, q, pi_Du, None, freeDu=0)

    schedule.add(
        ConditionalOperation(body=sub_schedule, qubit_name=q),
        rel_time=TRIGGER_DELAY,
    )

    return spec_pulse, schedule
...
#                     # active_RO, sched = active_reset(self.QD_agent, sched, q, acq_idx, qubit_idx, R_amp, R_duration, R_integration, R_inte_delay, pi_amp, pi_dura[q], active_RO, acq_threshold=self.QD_agent.quantum_device.get_element(q).measure.acq_threshold(),acq_rotation=self.QD_agent.quantum_device.get_element(q).measure.acq_rotation())
#                 else:
#                     sched.add(Reset(q))
...
#                 Integration(sched,q,R_inte_delay[q],R_integration,spec_pulse,acq_idx,acq_channel=qubit_idx,single_shot=singleshot,get_trace=False,trace_recordlength=0,RO_protocol="ThresholdedAcquisition" if self.QD_agent.activeReset else "SSBIntegrationComplex", acq_threshold=self.QD_agent.quantum_device.get_element(q).measure.acq_threshold(),acq_rotation=self.QD_agent.quantum_device.get_element(q).measure.acq_rotation())
...
class EnergyRelaxPS(ScheduleConductor):
    ...
    def __PulseSchedule__(self, 
        ...
        for acq_idx, freeDu in enumerate(freeduration):
            ...
            for qubit_idx, qubit in enumerate(join_qubits):
                if self.QD_agent.activeReset:
                    sched.add(
                        ConditionalReset(qubit, acq_index=acq_idx, acq_channel=qubit_idx),
                        label=f"Reset {qubit} {acq_idx}",
                    )
                else:
                    sched.add(Reset(qubit))

                sched.add(X(qubit), label=f"{qubit} pi  {acq_idx}")
            print(freeDu)
            sched.add(
                Measure(*join_qubits, acq_channel=linspace(0,len(join_qubits)), acq_index=acq_idx, bin_mode='append' if singleshot else 'average',acq_protocol='ThresholdedAcquisition' if self.QD_agent.activeReset else 'SSBIntegrationComplex'),
                ref_pt="start",
                rel_time=freeDu,
                label=f"Measurement {acq_idx}",
            )  
    ...
    def __SetParameters__(self, *args, **kwargs):
        ...
        self.__sched_kwargs = dict(
        freeduration=array(list(self._time_samples.values())[0]),
        join_qubits = list(self._time_samples.keys()),
        singleshot=self._os_mode,
        )
    ...
    def __Compose__(self, *args, **kwargs):
        ...
        if self._execution:
            ...
            for q in self._time_samples:
                preview_para = array([self._time_samples[q][0],self._time_samples[q][-1]])
            self.__sched_kwargs['freeduration']= preview_para
