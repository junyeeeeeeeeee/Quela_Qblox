...
from qblox.support.UserFriend import *
from utils.tutorial_utils import show_args
from qcodes.parameters import ManualParameter
from xarray import Dataset
from numpy import linspace, array, arange, NaN, ndarray, sin, mean, cos, moveaxis
from qblox.support import QDmanager, Data_manager, cds
from quantify_scheduler.gettables import ScheduleGettable
from quantify_core.measurement.control import MeasurementControl
from qblox.Configs.Readout_setup import get_manully_integration_time, get_manully_reset_time, set_reset_time_by_T1
from qblox.support.Path_Book import find_latest_QD_pkl_for_dr, meas_raw_dir
from qblox.support import init_meas, init_system_atte, shut_down, coupler_zctrl, compose_para_for_multiplexing, reset_offset
from qblox.support.Pulse_schedule_library import multi_PI_amp_cali_sche, set_LO_frequency, pulse_preview
from qblox.analysis.Multiplexing_analysis import Multiplex_analyzer
from qblox.analysis.raw_data_demolisher import piampcali_dataReducer


def half_pi_amp_cali(QD_agent:QDmanager,meas_ctrl:MeasurementControl, ro_elements:dict, pi_pair_num:list=[3,5],n_avg:int=300,run:bool=True,specific_data_folder:str=''):
    results = {}
    ...
    def pi_pair_dep_exe(pi_pair_num:int)->dict:
        ...
        if run:
            ...
            for q_idx, q in enumerate(ro_elements["amp_samples"]):
                ...
                dict_[f"{q}_HalfPIcoef"] = [list(ro_elements["amp_samples"][q])]*2
        else:
            preview_para = {}
            for q in ro_elements["amp_samples"]:
    ...
    if run:
        ...
        nc_path = Data_manager().save_raw_data(QD_agent=QD_agent,ds=dataset_2_nc,qb="multiQ",exp_type="xyl05cali",specific_dataFolder=specific_data_folder,get_data_loc=True)
   
    return nc_path

    

def halfpi_coef_calibrator(QD_agent:QDmanager,cluster:Cluster,meas_ctrl:MeasurementControl,Fctrl:dict,ro_elements:dict,pi_pair_num:list=[3,5],run:bool=True,avg_times:int=500,data_folder:str=''):
    if run:
        for q in ro_elements["amp_samples"]:
            Fctrl[q](float(QD_agent.Fluxmanager.get_proper_zbiasFor(q)))
        nc_path = half_pi_amp_cali(QD_agent,meas_ctrl,ro_elements,run=True,pi_pair_num=pi_pair_num,n_avg=avg_times,specific_data_folder=data_folder)
        reset_offset(Fctrl)
        cluster.reset()
        
        return nc_path
    else:
        _ = half_pi_amp_cali(QD_agent,meas_ctrl,ro_elements,run=False,pi_pair_num=pi_pair_num,n_avg=avg_times,specific_data_folder=data_folder)
        return 0
  
def halfpi_cali_waiter(QD_agent:QDmanager, ro_element:dict, amp_pts:int=100)->dict:
    """
    ro_elements = {"q0":{"falf_pi_coef_span":0.15,"xy_IF":250e6}} for example.
    """
    ...
    for q in ro_element :
        ...
        ro_elements["amp_samples"][q] = linspace(0.5-ro_element[q]["falf_pi_coef_span"],0.5+ro_element[q]["falf_pi_coef_span"],amp_pts) 
...
if __name__ == "__main__":
    ...
    ro_elements = {"q0":{"falf_pi_coef_span":0.15,"xy_IF":250e6},"q1":{"falf_pi_coef_span":0.15,"xy_IF":250e6}}
    couplers = []


    """ Optional paras """
    pi_pair_num:list = [1,2]
    data_pts:int = 80
    avg_n:int = 800
    ...
    ro_elements = halfpi_cali_waiter(QD_agent,ro_elements,data_pts)
    for q in ro_elements["amp_samples"]:
    ...
    """Running """
    nc_path = halfpi_coef_calibrator(QD_agent,cluster,meas_ctrl,Fctrl,ro_elements,run=execution,avg_times=avg_n,pi_pair_num=pi_pair_num)
